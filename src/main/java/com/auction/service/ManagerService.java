...
public class ManagerService {
    ...
    public void setupStartPrice(int itemId, int newPrice) throws Exception{     // hàm thiết lập giá khởi điểm
        try{
            Items item = itemService.getItemById(itemId);                          // gọi 1 sản phẩm trong kho theo ID
            if( item == null){
    ...
    public void scheduleAuction(int auctionId, int itemId, LocalDateTime startTime) throws Exception{  // hàm cài đặt thời gian
        try{
            Items item = itemService.getItemById(itemId);                           // lấy 1 sản phẩm trong kho sản phẩm theo ID
            ...
            // QUAN TRỌNG: Gán giá hiện tại bằng giá khởi điểm của món đồ
            newAuction.setCurrentPrice(item.getStartPrice());
