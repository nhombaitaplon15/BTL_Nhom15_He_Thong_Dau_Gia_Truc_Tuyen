...
public class BiddingHistoryController implements Initializable {
    ...
    private void loadHistory() {
        ...
        new Thread(() -> {
            try {
                // 1. SỬA TÊN HÀM: Gọi đúng hàm getHistoryByBidderId có sẵn trong DAO của nhóm bạn
                // Đồng thời hứng dữ liệu gốc là kiểu BiddingHistory từ DB
                List<com.auction.common.model.BiddingHistory> dbList = historyDAO.getHistoryByBidderId(currentUser.getId());

                // 2. CHUYỂN ĐỔI KIỂU DỮ LIỆU: Convert từ BiddingHistory sang BidHistoryRow để TableView hiểu được
                List<BidHistoryRow> uiList = dbList.stream().map(bh -> {
                    // Định dạng ngày giờ hiển thị dạng String cho đẹp mắt
                    String formattedTime = bh.getBidTime() != null ? bh.getBidTime().toString().replace("T", " ") : "";

                    return new BidHistoryRow(
                            bh.getId(),          // Mã lịch sử (Kế thừa từ Entity cha)
                            bh.getAuctionId(),   // Mã phiên
                            bh.getItemName(),    // Tên vật phẩm
                            bh.getBidAmount(),   // Số tiền đặt
                            formattedTime,       // Thời gian dạng String
                            bh.getStatus()       // Trạng thái ("Đang dẫn đầu", "Bị vượt mặt")
                    );
                }).collect(Collectors.toList());

                // 3. Đẩy dữ liệu đã đồng bộ lên giao diện JavaFX an toàn
                Platform.runLater(() -> {
                    historyList.setAll(uiList);
                    historyTable.setItems(historyList);
                });
            } catch (Exception e) {
                System.err.println("❌ Lỗi đồng bộ nạp lịch sử đấu giá!");
                e.printStackTrace();
            }
        }).start();
    }
