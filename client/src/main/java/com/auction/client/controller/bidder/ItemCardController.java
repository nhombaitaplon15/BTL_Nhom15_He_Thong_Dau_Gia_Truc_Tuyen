...
public class ItemCardController {
    ...
    // THÊM 2 BIẾN NÀY ĐỂ LƯU TRỮ TẠM THỜI
    private double currentPriceVal;
    ...
    public void setData(Item item, int auctionId, User user, double currentPriceVal, LocalDateTime endTime) {
        ...
        // LƯU LẠI GIÁ TRỊ ĐỂ DÙNG LÚC VÀO PHÒNG
    ...
    private void handleAuctionEndedNotification(Message message) {
        ...
        if (auctionId != this.currentAuctionId) return;
    ...
    void handleBidAction(ActionEvent event) {
        try {
            ...
            if (roomController != null) {
                // TẠO OBJECT AUCTION TẠM THỜI ĐỂ TRUYỀN VÀO PHÒNG
                Auction tempAuction = new Auction();
                tempAuction.setAuctionId(currentAuctionId);
                tempAuction.setItem(currentItem);
                tempAuction.setStartingPrice(currentItem.getStartingPrice());
                tempAuction.setCurrentPrice(currentPriceVal);
                tempAuction.setEndTime(endTime);

                // GỌI ĐÚNG 2 THAM SỐ NHƯ BẢN THIẾT KẾ MỚI
                roomController.loadAuctionDetail(tempAuction, currentUser);
            }
