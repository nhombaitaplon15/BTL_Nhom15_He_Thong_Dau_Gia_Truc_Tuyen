 ...
 * * ĐÃ ĐỒNG BỘ & GỘP LOGIC:
 * 1. Khôi phục các phòng RUNNING từ DB lên RAM khi khởi động Server.
 * 2. Tự động kích hoạt luồng quét tự động (AuctionBot-Thread) ngay khi tạo Service.
 * 3. Fix lỗi khóa ngoại trong scheduleAuction (dùng trực tiếp itemId thay vì item.getId()).
 * 4. Giữ lại đầy đủ các hàm bổ trợ: getAuctionItemsBySeller (DTO), removeAuctionFromCache, getAuctionById.
 * 5. Giữ nguyên logic đóng/mở phòng real-time qua Socket Server.
 */
public class ManagerService {
    ...
    public ManagerService(ItemService itemService) {
        ...
        // [Từ Bản 1] Khôi phục lại toàn bộ phòng Real-time cho các phiên đang chạy dưới DB khi Server khởi động lại
        ...
        // [Từ Bản 1] Kích hoạt Bot tự động quét trạng thái SQL định kỳ
        autoCloseAuction();
    }
    ...
    /** Tìm danh sách DTO kết hợp cả Item và Auction dành cho người bán */
    ...
    public void scheduleAuction(int itemId, LocalDateTime startTime, LocalDateTime endTime) {
        ...
        // ĐÃ FIX: Sử dụng trực tiếp tham số 'itemId' thay vì gọi 'item.getId()' đang bị trả về 0 nhằm tránh lỗi vi phạm khóa ngoại SQL
