...
public class AuctionDAO {
    ...
    /** [TỪ BẢN 2] Lấy danh sách DTO đấu giá của Người bán theo trạng thái và từ khóa tên vật phẩm */
    public List<AuctionItemDTO> getAuctionsBySellerStatusAndKeyword(int sellerId, String status, String keyword) {
        ...
        String sql = """
            SELECT i.*, a.* FROM public.items i 
            INNER JOIN public.auctions a ON i.item_id = a.item_id 
    ...
    /** [TỪ BẢN 2] Lấy danh sách DTO đấu giá đã kết thúc/hủy của Người bán kết hợp từ khóa */
    ...
    /** [ĐỒNG BỘ CHUẨN] Thêm mới phiên đấu giá (Bản 1 bổ sung current_winner_id, bản 2 rút gọn để chuẩn hóa DB)
     * Giữ cấu trúc bảng chuẩn theo logic khởi tạo ban đầu.
     */
    public boolean insertAuction(Auction a) {
        ...
        return executeUpdate(sql,
                ...
                a.getCurrentPrice(),
                a.getTotalBids(),
                a.getCurrentWinnerId(), // Hỗ trợ trường lưu winner ban đầu nếu có
    ...
    /** [TỪ BẢN 2] Cập nhật thời gian khi người dùng/quản trị chỉnh sửa phiên */
    public boolean updateAuction(Auction a) {
        String sql = "UPDATE public.auctions SET start_time = ?, end_time = ? WHERE auction_id = ?";
        return executeUpdate(sql,
                Timestamp.valueOf(a.getStartTime()),
                Timestamp.valueOf(a.getEndTime()),
                a.getAuctionId());
    ...
    /** * [TỐI ƯU LOGIC XÓA]: Xóa hoàn toàn phiên đấu giá ra khỏi hệ thống.
     * Để bảo vệ toàn vẹn dữ liệu, chỉ cho phép xóa các phiên ở trạng thái nháp hoặc bị chặn (BLOCKED).
     */
    public boolean deleteAuction(int auctionId) {
        return executeUpdate("DELETE FROM public.auctions WHERE auction_id = ? AND auction_status IN ('BLOCKED', 'WAITING_FOR_ADMIN')", auctionId);
    }
